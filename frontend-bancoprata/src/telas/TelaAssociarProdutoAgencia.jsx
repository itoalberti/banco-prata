...
import { urlAgencia, urlProduto } from './___TelaAssociarProdutoAgencia';
...
export default function TelaAssociarProdutoAgencia(props) {
  ...
  useEffect(() => {
    if (uf) {
      const cidadesFiltradas = todasCidades.filter((agencia) => agencia.uf === uf);
      const cidadesUnicas = [...new Set(cidadesFiltradas.map((agencia) => agencia.cidade))];
      setFiltrarCidades(cidadesUnicas);
    } else {
      const cidadesUnicas = [...new Set(todasCidades.map((agencia) => agencia.cidade))];
      setFiltrarCidades(cidadesUnicas);
    }
  }, [uf, todasCidades]);
  ...
  useEffect(() => {
    if (cidadeSelecionada) {
      setFiltrarEnderecos(todosEnderecos.filter((agencia) => agencia.cidade === cidadeSelecionada));
    } else {
